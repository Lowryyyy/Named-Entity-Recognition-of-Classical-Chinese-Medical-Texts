...
class BERTCRFModel(nn.Module):
    def __init__(self, bert_model_name: str, num_tags: int, dropout: float):
...
class BERTCRFNER:
    ...
    def train(self, train_sentences: List[Tuple[List[str], List[str]]], val_sentences: List[Tuple[List[str], List[str]]] = None):
        train_dataset = BERTCRFDataset(train_sentences, self.tokenizer, self.tag2id, self.config.MAX_SEQ_LEN)
        train_loader = DataLoader(train_dataset, batch_size=self.config.BATCH_SIZE, shuffle=True)
        
        val_loader = None
        if val_sentences:
            val_dataset = BERTCRFDataset(val_sentences, self.tokenizer, self.tag2id, self.config.MAX_SEQ_LEN)
